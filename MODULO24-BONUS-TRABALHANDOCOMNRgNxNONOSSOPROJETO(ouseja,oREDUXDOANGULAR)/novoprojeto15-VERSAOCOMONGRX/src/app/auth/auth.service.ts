...
///e vamos usar o value inicial de 'null', para essa data retornada por esse subject...
  ...
  // onCreateAccount(authData: {  ///inutilizado
  //   email: string;
  //   password: string;
      ...
      //   catchError(
        ...
        // (responseData) => {
          ...
          // this.autoLogout(+responseData.expiresIn);
                ...
                // this.handleUser(responseData); //código com NGRX EFFECTS..
  ...
  // onLogin(authData: { ///inutilizado (agora usamos ngrx/effects)....
  //   email: string;
  //   password: string;
  //   returnSecureToken: boolean;
  // }) {
  ...
  //     .post<AuthResponseData>(
      ...
      //     (responseData) => {
            ...
            // this.handleUser(responseData); ///trocamos pelo uso de EFFECTS DO NGRX...
  ...
  // private storeUserToken(responseData: AuthResponseData) { ///inutilizado.
  ...
  // autoLogin() { ///COM O USO DE NGRX, mas sem o uso de NGRX/EFFECTS...
  ...
  //os 2 methods abaixo serão USADOS LÁ NO 'auth.effects.ts'...


  setLogoutTimer(expirationTime: number) {  //define um TIMER para então FAZER LOGOUT do user (quando o tempo acabar).... em milissegundos...
